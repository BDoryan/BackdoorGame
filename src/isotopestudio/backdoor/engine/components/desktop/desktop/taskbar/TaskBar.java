...
public class TaskBar extends Panel implements IComponent {
	...
	public void init() {
		...
		if (isHorizontal()) {
			...
			/*
			menu_button.getStyle().setTop(-2);
			menu_button.getStyle().setLeft(4);
			...
			menu_button.getListenerMap().addListener(MouseClickEvent.class, new MouseClickEventListener() {
				...
				public void process(MouseClickEvent event) {
					...
					IconLabel settings = new IconLabel(Lang.get("taskbar_settings"), BackdoorGame.getDatapack().getImage("icon_settings"), new LabelSizeEvent() {
						@Override
					...
					IconLabel profile = new IconLabel(Lang.get("taskbar_profile"), BackdoorGame.getDatapack().getImage("icon_profile"), new LabelSizeEvent() {
						@Override
						public void process(float width, float height) {
							popupmenu.initSize();
						}
					...
					IconLabel shutdown = new IconLabel(Lang.get("taskbar_shutdown"), BackdoorGame.getDatapack().getImage("icon_shutdown"), new LabelSizeEvent() {
						@Override
						public void process(float width, float height) {
							popupmenu.initSize();
						}
					...
					IconLabel restart = new IconLabel(Lang.get("taskbar_restart"), BackdoorGame.getDatapack().getImage("icon_shutdown"), new LabelSizeEvent() {
						@Override
						public void process(float width, float height) {
							popupmenu.initSize();
						}
			...
			box.getStyle().setLeft(25 + (menu_button.getStyle().getDisplay() == DisplayType.FLEX ?(float) menu_button.getStyle().getWidth().get() : menu_button.getSize().x));
			...
			Responsive responsive = new Responsive(friends_icon, (float)8, null, (float)8, null);
	...
	public void addWindowIcon(Window window) {
		...
		TaskBarWindowIcon window_icon = new TaskBarWindowIcon(window, window.hasIcon() ? window.getIcon().getImage() : BackdoorGame.getDatapack().getImage("window_icon_default"), width, width);	
	...
	public void updateIcon(Window window) {
		if(windows_icons.containsKey(window.getUuid().toString())) {
			windows_icons.get(window.getUuid().toString()).setIcon(window.hasIcon() ? window.getIcon().getImage() : BackdoorGame.getDatapack().getImage("window_icon_default"));
			return;
		} else {
