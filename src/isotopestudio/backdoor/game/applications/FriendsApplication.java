...
public class FriendsApplication extends Window implements IComponent {
	...
	public FriendsApplication() {
		...
		search_textfield.getListenerMap().addListener(KeyEvent.class, new KeyEventListener() {
			...
			public void process(KeyEvent event) {
				...
				if(event.getKey() == Keyboard.getNativeCode(KeyCode.ENTER) || event.getKey() == Keyboard.getNativeCode(KeyCode.NUMPAD_ENTER)) {
					searchPlayer(search_textfield.getTextState().getText());
				}
			}
		});
		search_textfield.getListenerMap().addListener(TextInputContentChangeEvent.class, new TextInputContentChangeEventListener() {
			long last_type = 0l;
			@Override
			public void process(TextInputContentChangeEvent event) {
				...
				new Timer().schedule(new TimerTask() {
					@Override
					public void run() {
						System.out.println((System.currentTimeMillis() - last_type));
						if((System.currentTimeMillis() - last_type) >= 200) {
							if(event.getNewValue().equals(""))return;
							searchPlayer(event.getNewValue());
						}
					}
				}, 200);*/
			}
		...
		new Timer().schedule(new TimerTask() {
			...
			public void run() {
				try {
					...
					ArrayList<Profile> friends_ = new FriendsAPI().getListRequest(BackdoorGame.getUser().getEmail(), BackdoorGame.getUser().getToken());
					...
					ArrayList<FriendComponent> friends_ = new ArrayList<>();
					if(friends.size() > 0) {
						friends_.addAll(friends);
						for (FriendComponent friend : friends_) {
							friend.setProfile(new ProfileAPI().getProfile(BackdoorGame.getUser().getEmail(),
									BackdoorGame.getUser().getToken(), friend.getProfile().getUuidString()));
							friend.update();
						}
					}

					// refresh request friends list
					if(friendsRequest.size() < 0 )return;
					ArrayList<FriendRequestComponent> friendsRequest_ = new ArrayList<>();
					friends_.addAll(friends);
					for (FriendRequestComponent friend : friendsRequest_) {
						friend.setProfile(new ProfileAPI().getProfile(BackdoorGame.getUser().getEmail(),
								BackdoorGame.getUser().getToken(), friend.getProfile().getUuidString()));
						friend.update();
					}*/
	...
	private void searchPlayer(String target) {
		...
		try {
			for (Profile friend : new FriendsAPI().getSearchRequest(BackdoorGame.getUser().getEmail(),
				...
				if(new FriendsAPI().isYourFriend(BackdoorGame.getUser().getEmail(),
					BackdoorGame.getUser().getToken(), friend.getUuidString())) {
	...
	private void loadFriendsList() {
		...
		friends_panel.getContainer().clearChildComponents();
		try {
			loadFriends(new FriendsAPI().getFriends(BackdoorGame.getUser().getEmail(),
	...
	private class FriendComponent extends Panel {
		...
		public FriendComponent(Profile profile) {
			...
			profile_status.getListenerMap().addListener(LabelWidthChangeEvent.class, new LabelWidthChangeEventListener() {
				@Override
				public void process(LabelWidthChangeEvent event) {
			...
			getListenerMap().addListener(MouseClickEvent.class, new MouseClickEventListener() {
				...
				public void process(MouseClickEvent event) {
					if(event.getAction() == MouseClickAction.RELEASE && event.getButton() == MouseButton.MOUSE_BUTTON_2) {
						PopupMenu popupmenu = new PopupMenu((int)event.getAbsolutePosition().x, (int)event.getAbsolutePosition().y);
						...
						MouseClickEventListener delete_click = new MouseClickEventListener() {
							...
							public void process(MouseClickEvent event) {
								...
								try {
									new FriendsAPI().removeRequest(BackdoorGame.getUser().getEmail(), BackdoorGame.getUser().getToken(), FriendComponent.this.profile.getUuidString());
									loadFriendsList();
								} catch (ClientProtocolException e) {
									e.printStackTrace();
								} catch (IOException e) {
		...
		public FriendRequestComponent(Profile profile) {
			...
			delete_label.getListenerMap().addListener(MouseClickEvent.class, new MouseClickEventListener() {
				...
				public void process(MouseClickEvent event) {
					if(event.getAction() == MouseClickAction.RELEASE) {
						try {
							new FriendsAPI().declineRequest(BackdoorGame.getUser().getEmail(), BackdoorGame.getUser().getToken(), profile.getUuidString());
							loadFriendsList();
						} catch (ClientProtocolException e) {
							e.printStackTrace();
						} catch (IOException e) {
			...
			accept_label.getListenerMap().addListener(MouseClickEvent.class, new MouseClickEventListener() {
				...
				public void process(MouseClickEvent event) {
					if(event.getAction() == MouseClickAction.RELEASE) {
						try {
							new FriendsAPI().acceptRequest(BackdoorGame.getUser().getEmail(), BackdoorGame.getUser().getToken(), profile.getUuidString());
							loadFriendsList();
						} catch (ClientProtocolException e) {
							e.printStackTrace();
						} catch (IOException e) {
		...
		public FriendSearchComponenn(Profile profile) {
			...
			add_label.getListenerMap().addListener(MouseClickEvent.class, new MouseClickEventListener() {
				...
				public void process(MouseClickEvent event) {
					if(event.getAction() == MouseClickAction.RELEASE) {
						try {
							new FriendsAPI().sentRequest(BackdoorGame.getUser().getEmail(), BackdoorGame.getUser().getToken(), profile.getUuidString());
