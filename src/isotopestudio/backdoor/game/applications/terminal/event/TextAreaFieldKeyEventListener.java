package isotopestudio.backdoor.game.applications.terminal.event;
...
public class TextAreaFieldKeyEventListener implements EventListener<KeyboardEvent> {
	...
	public TextAreaFieldKeyEventListener(TerminalApplication terminal) {
	...
	public void process(KeyboardEvent event) {
		...
		if (keyCode == KeyCode.LEFT_SHIFT || keyCode == KeyCode.RIGHT_SHIFT || keyCode == KeyCode.LEFT_CONTROL
				|| keyCode == KeyCode.RIGHT_CONTROL || keyCode == KeyCode.LEFT_ALT || keyCode == KeyCode.RIGHT_ALT)
			return;
		...
		if (!pressed) {
			EventProcessorProvider.getInstance()
					.pushEvent(new TextAreaFieldUpdateEvent(textAreaField, event.getContext(), event.getFrame()));
			return;
		}
		...
		if (!oldText.equals(newText)) {
			EventProcessorProvider.getInstance()
					.pushEvent(new TextAreaFieldContentChangeEvent<>((TextAreaField) event.getTargetComponent(),
	...
	/**
	 * Delete action. Used to delete selected text or symbol after caret or word
	 * after caret.
	 *
	...
	private void keyDeleteAction(TextAreaField gui, Set<KeyMod> mods) {
		if (gui.isEditable()) {
			if(terminal.getCommand().equals(""))return;
			...
			caretPosition = caretPosition - (terminal.textContent().length() - terminal.getCommand().length());
			if(caretPosition == terminal.getCommand().length()) {
				terminal.setCommand(terminal.getCommand().substring(0, caretPosition));
			} else {
				terminal.setCommand(terminal.getCommand().substring(0, caretPosition) + (terminal.getCommand().substring(caretPosition + 1)));	
			}
		}
	}

	/**
	 * Backspace action. Deletes selected text or symbol before caret or words
	 * before caret.
	 *
	...
	private void keyBackSpaceAction(TextAreaField gui, Set<KeyMod> mods) {
		if (gui.isEditable()) {
			...
			terminal.setCommand(terminal.getCommand().substring(0, terminal.getCommand().length() - 1));
		}
	}
		...
		if (caretPosition > 0) {
			...
			if (some.lineIndex > 0) {
				...
				newCaretPosition = caretPosition
						- (some.caretPositionInLine >= nl - 1 ? some.caretPositionInLine + 1 : nl);
			} else {
				newCaretPosition = 0;
			}
	...
	private void keyLeftAction(TextAreaField gui, Set<KeyMod> mods) {
		...
		if(newCaretPosition < terminal.textContent().length() - terminal.getCommand().length()) {
			newCaretPosition = terminal.textContent().length() - terminal.getCommand().length();
		}
